// Home page. Renders the mirrored HTML with optional Payload-backed
// content injected into marked zones (hero, about, service cards, CTA).
// On-demand revalidation: edits in the admin call revalidateTag('page:/')
// so the public site picks up the new content within one request.
...
const SOURCE = resolveMirrorPath("index.html");
const PAGE_URL = "/";
...
async function loadAllServices(): Promise<Record<string, Service>> {
	try {
		...
		for (const s of result.docs as Service[]) {
			map[s.slug] = s;
		}
		return map;
	} catch {
